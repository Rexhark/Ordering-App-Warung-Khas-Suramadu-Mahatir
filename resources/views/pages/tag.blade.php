        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-button">
                    <a href="{{ url('dashboard/tag/tambah') }}" class="btn btn-primary">Tambah</a>
                </div>
                <div class="section-header-breadcrumb">
                    <div class="breadcrumb-item"><a href="{{ url('dashboard') }}">Dashboard</a></div>
                    <div class="breadcrumb-item active">Tag</a></div>
                </div>
            </div>
            ...
            <div class="section-body">
                <h2 class="section-title">Daftar Tag</h2>
                <p class="section-lead">Berikut merupakan daftar tag.</p>
                <div class="row">
                    <div class="col">
                        <div class="card mb-0">
                            <div class="card-body">
                            ...
                            <div class="tab-pane fade show active" id="bydatetab" role="tabpanel" aria-labelledby="by-date-tab">
                                <div class="table-responsive">
                                    <table class="table-bordered table-md table">
                                        ...
                                        @foreach ($tagByDate as $t1)
                                        <tr>
                                            <td>{{$loop->iteration}}</td>
                                            <td>{{$t1->name}}</td>
                                            @if ($t1->description = null or strip_tags($t1->description = ''))
                                            <td class="text-center">{{strip_tags($t1->description)}}</td>
                                            @else
                                            <td class="text-center">Tidak ada deskripsi</td>
                                            @endif
                                            <td class="text-center p-0">{{$t1->created_at->toDateString()}}</td>
                                            <td>
                                                <div class="container-fluid row m-0 px-0">
                                                <a href="{{ url('dashboard/tag/edit/'.$t1->name) }}" class="btn btn-warning col mx-2">Edit</a>
                                                <button type="button" class="col btn btn-danger mx-2" data-toggle="modal" data-target="#hapusdata{{$t1->id}}">Hapus</button>
                                                {{-- Modal Delete --}}
                                                <div class="modal fade" id="hapusdata{{$t1->id}}" tabindex="-1" aria-labelledby="hapusdata{{$t1->id}}" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="hapusdata{{$t1->id}}">Konfirmasi Hapus Tag</h5>
                                                                ...
                                                                <form action="tag/hapus" method="post">   
                                                                    @csrf
                                                                    @method('delete')
                                                                    <input type="text" name="id" class="form-control" value="{{$t1->id}}" hidden>
                            ...
                            <div class="tab-pane fade" id="bymostusedtab" role="tabpanel" aria-labelledby="by-most-used-tab">
                                <div class="table-responsive">
                                    <table class="table-bordered table-md table">
                                        ...
                                        @foreach ($tagByMostUsed as $c2)
                                        <tr>
                                            <td>{{$loop->iteration}}</td>
                                            <td>{{$c2->name}}</td>
                                            @if ($c2->description = null or strip_tags($c2->description = ''))
