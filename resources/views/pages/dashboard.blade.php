                                ...
                                @foreach ($mostFavFood as $food)
                                    <div class="col text-center">
                                        <div class="gallery mb-3">
                                            <img alt="Gambar" src="{{ asset('storage/' . $food->image) }}"
                                                class="img-fluid rounded-circle">
                                        </div>
                                        <span class="badge badge-primary p-1 pl-3">Suka: <span
                                                class="badge badge-white">{{ $food->like_count }}</span></span>
                            ...
                            <div class="row">
                                @foreach ($mostOrderedFood as $food)
                                    <div class="col text-center">
                                        ...
                                        <span class="badge badge-primary p-1 pl-3">Order: <span
                                                class="badge badge-white">{{ $food->ordered }}</span></span>
                                        <div class="font-weight-bold mt-2">{{ $loop->iteration }}. {{ $food->name }}</div>
                                    </div>
                                @endforeach
                            </div>
                                    ...
                                    <tbody class="text-start">
                                        @foreach ($order as $o)
                                            <tr>
                                                ...
                                                <td>{{ $o->user->name }}</td>
                                                <td>
                                                    @foreach ($o->orderDetails()->get() as $od)
                                                        <ul>{{ $od->food->name }} x {{ $od->quantity }} (Rp{{ number_format($od->food->price ?? 0, 0, ',', '.') }})</ul>
                                                    @endforeach
                                                </td>
                                                <td>Rp{{ number_format($o->totalPayment ?? 0, 0, ',', '.') }}</td>
