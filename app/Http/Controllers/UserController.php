...
class UserController extends Controller
{
    ...
    public function daftarMain(Request $request)
    {
        ...
        if ($request->file('image')) {
            $validateData['image'] = $request->file('image')->store('profil', 'public');
        } else {
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            // Hapus cookie setelah data dipindahkan ke database
            // cookie()->queue(cookie()->forget('liked_items'));
            ...
            if (!empty($cart)) {
                ...
                // Update total pembayaran pada order
                $order->totalPayment += array_sum(array_column($cart, 'totalPrice'));
                $order->save();
            }
